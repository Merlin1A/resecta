...
// Packet DOB label-anchored role (retirement checklist, ROLE 2 of 3).
//
// The packet successor to `RealDocDOBLabelAnchoredTests`. That suite proved the
// label-anchored DOB path fires on the retired fixture's
...
// the retired fixture can be dropped with no DOB-label coverage gap.
//
...
// ("Born March 14, 1985", URLA-B / page index 1, confirmed must_fire); the
// numeric URLA DOBs reconciled to should_fire (they miss the text leg on
...
//
// MATCHED-TEXT LOGGING: synthetic, publicly-manifested fixture -- a single
...
@Suite("Packet DOB label-anchored role (retirement checklist, counts)", .serialized)
    ...
    func labelAnchoredDOBFiresOnPacket() async throws {
        ...
        for pageIndex in 0..<document.pageCount {
            ...
            for match in dobMatches {
                // Label path emits the fixed 0.85.
        ...
        // Counts + page attribution (a matched-text line is permitted here).
        ...
        // Posterior threshold gate relationship (identical to the predecessor sibling): the fixed 0.85
