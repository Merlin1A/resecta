...
// merged(overriding:) layers per-category overrides on top of the
// preset vector. Non-calibration categories are silently dropped;
// values are clamped to [0, 1].

@Suite("PresetThresholdVector.merged(overriding:)")
struct PresetThresholdVectorMergeTests {
    ...
    // email / phone / creditCard now have wire names, so they ARE
    // applied by merged(overriding:). The test verifies the new behavior:
    // all three overrides are reflected in the merged vector.
    @Test("email / phone / creditCard now have wire names — overrides are applied")
    ...
    // email now has a wire name, so all three overrides apply.
    @Test("Mixed override with email — all three applied")
    func mixedOverrideAllApplied() {
        let merged = baseline.merged(overriding: [
            .ssn: 0.95,      // applied (was calibration before)
            .email: 0.50,    // applied (now has wire name)
        ...
        #expect(merged.threshold(for: .email) == 0.50,
                "email override must be applied")
    }
