...
// wireName extensions for 8 previously ungated categories, plus
// builtInDefaults rows and threshold-gate integration.

@Suite("PresetThresholdVector wireName + threshold gate")
struct PresetThresholdVectorTests {
    ...
    // MARK: - Threshold-gate integration: EIN threshold gate

    /// The threshold gate with balanced ein=0.55: an EIN match at score 0.50 must be
    ...
    // MARK: - Threshold-gate residue guard

    /// Permanent regression guard for the threshold-gate nil-bypass residue:
    /// every wire-named PIICategory must have a non-nil threshold in every
    /// preset of `builtInDefaults`. A wire-named category without a row would
    /// make the gate read a nil cutoff and pass the category through ungated.
    /// Pairs with the debug-only completeness assertion in `builtInDefaults`.
    @Test("All wireName-mapped categories have a non-nil builtInDefaults threshold")
        ...
        for category in PIICategory.allCases {
            ...
            for (preset, vector) in bundle.presets {
                #expect(
                    ...
                    "Wire-named \(category) lacks a \(preset) builtInDefaults threshold — the threshold gate would read a nil cutoff and pass it through ungated")
            }
        }
    }

    // MARK: - Preset-cutoff invariant (guards 28921a52 against a threshold-lowering "fix")

    /// The NPI and DOB margin fixes live entirely in detector Swift literals;
    /// the calibrated preset blob (git 28921a52) is NOT touched. This pins the
    /// shipped npi/dob cutoffs at their real values so a future "lower the
    /// gate" change that churns the blob trips here and is reviewed. NOTE:
    /// conservative.npi is 0.85 (NOT 0.602) — it intentionally requires the
    /// keyword boost; 0.602 is the balanced + aggressive cutoff.
    ...
    func presetNPIAndDOBCutoffsAreInvariant() {
        ...
        // npi cutoffs (gate the bare-NPI margin)
        ...
        // dob cutoffs (gate the labeled-DOB margin)
