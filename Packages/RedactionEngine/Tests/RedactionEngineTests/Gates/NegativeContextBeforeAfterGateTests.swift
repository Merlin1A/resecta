...
//
// Each method asserts its G8 sweep produced non-empty per-category/doctype
// cells and prints a sample of them (see printSummary) — the gate's load-
// bearing check that the corpus fixture and detector wiring are live.
//
...
// they are never suppressed; scoring them would add only noise.
...
struct NegativeContextBeforeAfterGateTests {
    ...
    func testBeforeConfiguration() async throws {
        ...
        let report = try await runGate(detector: detector)
        // The before run must produce non-trivial output.
        #expect(!report.by_category_doctype.isEmpty,
    ...
    private func runGate(detector: PIIDetector) async throws -> GateG8Results {
        try await sweepG8Corpus(detector: detector)
    }
    ...
    private func printSummary(_ report: GateG8Results, label: String) {
        ...
        for key in sampleKeys {
            if let c = report.by_category_doctype[key] {
        ...
        print("[D1 \(label)] total G8 cells: \(report.by_category_doctype.count)")
    }
}
