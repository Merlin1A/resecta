...
// G8 anchoring-rate regression guard for the federal_agency rebuild
// cutover. Loads `g8_corpus.json`, walks every `.foia` + `.medical`
// document, runs `InstitutionGazetteer.findInstitution(in:)`
...
// pre-cutover commit (institutions.json count: 1343 entries pre-cutover).
//
// Regression model: when the rebuilt institutions.json (~470 entries,
// cutover from federalregister_agencies.json) lands, this test re-runs
// and the floors below flag any silent recall drop on FOIA / medical
// strata.
//
// Rationale:
...
//
// Both floors were recorded at the pre-cutover measurement; the
// post-cutover re-run compares against the values below. If the rebuild
// drops a load-bearing entry the per-doctype rate slips and this test
// fails — visible regression detection instead of silent recall
// degradation.
//
...
@Suite("InstitutionGazetteer × G8 anchoring rate")
struct InstitutionGazetteerAnchoringRateTests {

    // MARK: - Baseline floors (pre-cutover)

    /// Pre-cutover FOIA-stratum hit-rate floor. Measured at the pre-cutover
    /// commit against institutions.json (1343 entries): **0/150 = 0.0000**.
    /// The surprising zero baseline reflects a G8 fixture characteristic —
    /// the V1 FOIA templates don't name federal agencies in their headers
    ...
    /// A V1.1+ followup could expand the FOIA fixture set to include
    /// agency letterheads.
    static let foiaHitRateFloor: Double = 0.0

    /// Pre-cutover medical-stratum hit-rate floor. Measured at the
    /// pre-cutover commit against institutions.json (1343 entries):
    ...
    /// This is the strongest regression guard here: if the rebuilt
    /// institutions.json drops the DEA entry (or its "DEA" alias), the
    /// medical-stratum rate slips and this test fails — visible
    /// post-cutover detection.
    ...
    func foiaAnchoringRate() throws {
        ...
        print("[anchoring-rate baseline FOIA] docs=\(stats.docCount) hits=\(stats.hitCount) " +
    ...
    func medicalAnchoringRate() throws {
        ...
        print("[anchoring-rate baseline medical] docs=\(stats.docCount) hits=\(stats.hitCount) " +
              "hitRate=\(String(format: "%.4f", stats.hitRate)) " +
              "anchoredRate=\(String(format: "%.4f", stats.anchoredDoctypeRate)) " +
              "uniqueInstitutions=\(stats.uniqueInstitutionsMatched)")
    }
