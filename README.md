...
The test tree is larger than the source tree: roughly 61,000 lines of Swift source to roughly 90,000 lines of test code, about 1.5×. Counted from the current tree:

- **Engine package** (`Packages/RedactionEngine/Tests`) — 1,767 Swift Testing `@Test` functions across 236 suites: the pipeline and rasterization, the verification layers, the security suites (fake redaction, pixel destruction, rotated-page coordinates, adversarial verification), search, detection, and the corpus measurement harnesses.
