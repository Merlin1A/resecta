...
// Audit schema selector.
//
// The confirmation dialog routes its share `Button`s through
...
// is out of scope for now; the previously pre-staged disabled `.v3`
// stub was removed. The `AuditSchemaVersion` enum carries
// the metadata-value mapping so the selector is pinned without a
// SwiftUI host.

@Suite("Audit schema selector", .tags(.search))
@MainActor
struct AuditSchemaPickerTests {
    ...
    func defaultSchemaIsV4() {
        ...
        // Renaming this assertion is a deliberate spec-edit event.
