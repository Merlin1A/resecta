...
struct ScanReviewAbsorptionTests {
    ...
    // MARK: - addSelections(where:) additive union (RB-21/UXC-12)

    @Test("addSelections(where:) unions into existing selections without deselecting")
    func addSelectionsUnionsWithoutDeselecting() {
        let high = makeDetection(page: 0, confidence: 0.95)
        let low = makeDetection(page: 0, kind: .pii(.email), confidence: 0.6, matchedText: "j@x.com")
        let flat = ScanReviewSection.flattenedFindings([0: [high, low]])
        // A prior manual pick on `low`, outside the new predicate.
        let existing: [UUID: Bool] = [low.id: true]

        let selections = ScanReviewSection.addSelections(
            where: { $0.confidence >= 0.9 }, in: flat, to: existing
        )

        #expect(selections[high.id] == true)
        #expect(selections[low.id] == true,
                "a prior true entry outside the predicate must survive the union")

        // Idempotent: re-applying the same predicate over its own output
        // changes nothing.
        let again = ScanReviewSection.addSelections(
            where: { $0.confidence >= 0.9 }, in: flat, to: selections
        )
        #expect(again == selections)
    }

    @Test("addSelections(where:) applied twice with different predicates yields the selection-count union")
    func addSelectionsCountIsUnionAcrossPredicates() {
        let ssnHigh = makeDetection(page: 0, confidence: 0.95) // matches both predicates
        let emailMid = makeDetection(page: 0, kind: .pii(.email), confidence: 0.8, matchedText: "j@x.com")
        let phoneLow = makeDetection(page: 1, kind: .pii(.phone), confidence: 0.3, matchedText: "(555) 010-2934")
        let flat = ScanReviewSection.flattenedFindings([0: [ssnHigh, emailMid], 1: [phoneLow]])

        // Predicate 1 ("≥ 90%"): matches ssnHigh only.
        let afterFirst = ScanReviewSection.addSelections(
            where: { $0.confidence >= 0.9 }, in: flat, to: [:]
        )
        #expect(afterFirst.values.count { $0 } == 1)

        // Predicate 2 ("≥ 75%"): matches ssnHigh (overlap) AND emailMid (new).
        let afterSecond = ScanReviewSection.addSelections(
            where: { $0.confidence >= 0.75 }, in: flat, to: afterFirst
        )

        // Mirrors `SearchAndRedactSheet.reviewAcceptedCount`'s exact computation.
        let acceptedCount = afterSecond.values.count { $0 }
        #expect(acceptedCount == 2,
                "the selection count is the UNION of both predicates' matches (ssnHigh, emailMid) — the overlapping ssnHigh is not double-counted")
        #expect(afterSecond[ssnHigh.id] == true)
        #expect(afterSecond[emailMid.id] == true)
        #expect(afterSecond[phoneLow.id] == false)
    }

    @Test("addSelections(where:) drops a stale id no longer present in findings (matches the old replace-write's pruning)")
    func addSelectionsDropsStaleIDs() {
        let current = makeDetection(page: 0, confidence: 0.95)
        let flat = ScanReviewSection.flattenedFindings([0: [current]])
        // `existing` carries a stale id from a detection no longer part
        // of the staged set (e.g. already promoted/pruned elsewhere).
        let staleID = UUID()
        let existing: [UUID: Bool] = [staleID: true, current.id: false]

        let selections = ScanReviewSection.addSelections(
            where: { $0.confidence >= 0.9 }, in: flat, to: existing
        )

        #expect(selections[staleID] == nil,
                "a stale id absent from findings must be pruned, not merged forward")
        #expect(selections[current.id] == true)
